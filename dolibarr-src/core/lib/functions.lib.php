...
### helperpour la construction d'url des documents privés
function dolGetDocumentPrivateUrl($urlwithroot, $modulepart, $filepath, $entity = 0)
{
	$rellink = '/document.php?modulepart=' . $modulepart . '&attachment=1';
	if (!empty($entity)) {
		$rellink .= '&entity='.$entity;
	}
	$rellink .= '&file='.urlencode($filepath);

	return $urlwithroot.$rellink;
}
