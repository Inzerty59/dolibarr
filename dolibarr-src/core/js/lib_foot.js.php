...
print '
	jQuery(document).ready(function() {
				...
				jQuery(document).on(\'click\', \'.clipboardCPValue, .clipboardCPButton, .clipboardCPValueToPrint\', function(event) {
					...
					var clicked = this;
					var lastparent = $(clicked).parent();				/* .parent is clipboardCP */
					var lastchild = clicked.parentNode.lastChild;		/* .parentNode is clipboardCP and last child is clipboardCPText */
					var tmp = lastchild.innerHTML;
					var valueToCopy = lastparent.children(".clipboardCPValue").text();

					function showClipboardCopyResult(succeed) {
						if (succeed) {
							lastparent.children(".clipboardCPButton").hide();
							lastparent.children(".clipboardCPTick").css("display", "inline-block");	/* better than .show() because the show set the display to "inline" */
						...
						setTimeout(() => {
							lastchild.innerHTML = tmp;
							lastparent.children(".clipboardCPTick").hide();
							if (!lastparent.hasClass("clipboardCPShowOnHover") || lastparent.is(":hover")) {
								lastparent.children(".clipboardCPButton").css("display", "inline-block");
							}
						}, 2000);
					}
                    ...
                    showClipboardCopyResult(fallbackCopyToClipboard(valueToCopy));
