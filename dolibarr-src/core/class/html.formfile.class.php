			...
			foreach ($filearray as $key => $file) {      // filearray must be only files here
				if ($file['name'] != '.' && $file['name'] != '..' && !preg_match('/\.meta$/i', $file['name'])) {
					...
					if (!$editline) {
						...
						$urltowholeresource = dolGetDocumentPrivateUrl(DOL_MAIN_URL_ROOT, $modulepart, $filepath, (empty($object->entity) ? $conf->entity : $object->entity));
						print showValueWithClipboardCPButton($urltowholeresource, 0, 'none');
