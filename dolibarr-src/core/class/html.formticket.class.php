...
class FormTicket
{
    ...
    public function renderEcmAttachmentDialog($htmlname = 'ecm_file_rowid')
	{
		global $user;
		...
		$arbo = $ecmdirstatic->get_full_arbo(1);
		if (is_array($arbo)) {
			foreach ($arbo as $dir) {
				$dirs[(int) $dir['id']] = $dir;
			}
		...
		$filesbydir = array();
		$sql = 'SELECT rowid, filepath, filename';
		$sql .= ' FROM '.MAIN_DB_PREFIX.'ecm_files';
		$sql .= ' WHERE entity IN ('.getEntity('ecm_files').')';
		$sql .= " AND filepath LIKE 'ecm%'";
		$sql .= ' ORDER BY filepath, filename';
		...
		$out .= '<div class="ged-attach-head"><span>Choisir un fichier GED</span><button type="button" class="button smallpaddingimp" id="ged_modal_close">Fermer</button></div>';
		$out .= '<div class="ged-attach-body">';

		$out .= '<div class="ged-folder-list">';
		$out .= '<div class="ged-folder-title">Répertoires</div>';
		foreach ($dirs as $dir) {
			$path = empty($dir['fullrelativename']) ? 'ecm' : 'ecm/'.$dir['fullrelativename'];
			$level = empty($dir['level']) ? 0 : (int) $dir['level'];
			$out .= '<button type="button" class="ged-folder-entry" data-ged-path="'.dol_escape_htmltag($path).'">';
			$out .= str_repeat('&nbsp;&nbsp;&nbsp;', $level).dol_escape_htmltag($dir['label']);
			$out .= '</button>';
		}
		$out .= '</div>';

		$out .= '<div class="ged-file-list">';
		$out .= '<div class="ged-file-help">Sélectionnez un répertoire.</div>';
		$out .= '<div class="ged-file-empty" style="display: none;">Aucun fichier dans ce dossier.</div>';
		foreach ($filesbydir as $path => $files) {
			foreach ($files as $file) {
				$out .= '<button type="button" class="ged-file-row" style="display: none;" data-ged-dir="'.dol_escape_htmltag($path).'" data-ged-id="'.((int) $file->rowid).'" data-ged-name="'.dol_escape_htmltag($file->filename).'">';
				$out .= img_mime($file->filename).' '.dol_escape_htmltag($file->filename);
				$out .= '</button>';
			}
		}
		$out .= '</div>';
	...
	public function showForm($withdolfichehead = 0, $mode = 'edit', $public = 0, $with_contact = null, $action = '', $object = null)
	{
		...
		if (!empty($this->withfile)) {
			...
			if ($this->withfile == 2) { // Can add other files
				...
				$out .= '<div class="attach-file-line">';
				$out .= '<div class="attach-file-display">';
				$out .= '<input type="button" id="attach_source_toggle" value="Choisir un fichier">';
				$out .= '<span id="attach_selected_filename" class="attach-file-name">Aucun fichier choisi</span>';
				$out .= '</div>';

				$out .= '<input type="file" class="hidden" id="addedfile" name="addedfile" value="'.$langs->trans("Upload").'" />';
				...
				$out .= '<script nonce="'.getNonce().'" type="text/javascript">
				jQuery(document).ready(function () {
					function showGedFiles(path) {
						jQuery(".ged-file-row").hide();
						var rows = jQuery(".ged-file-row").filter(function () {
							return jQuery(this).attr("data-ged-dir") === path;
						});
						jQuery(".ged-file-help").hide();
						rows.show();
						jQuery(".ged-file-empty").toggle(rows.length === 0);
					}

					jQuery("#attach_source_toggle").on("click", function () {
						jQuery("#attach_source_choices").toggle();
					});

					jQuery("#attach_from_pc").on("click", function () {
						jQuery("#attach_source_choices").hide();
						jQuery("#ecm_file_rowid").val("0");
						jQuery("#addfile").attr("name", "addfile");
						jQuery("#addedfile").trigger("click");
					});

				jQuery("#attach_from_ged").on("click", function () {
					jQuery("#attach_source_choices").hide();
					jQuery("#attach_ged_modal").removeClass("hidden");
					jQuery(".ged-folder-entry").removeClass("active");
					jQuery(".ged-file-row").hide();
					jQuery(".ged-file-empty").hide();
					jQuery(".ged-file-help").show();
				});

					jQuery("#ged_modal_close, #attach_ged_modal").on("click", function (e) {
						if (e.target === this) {
							jQuery("#attach_ged_modal").addClass("hidden");
						}
					});

					jQuery(".ged-folder-entry").on("click", function () {
						jQuery(".ged-folder-entry").removeClass("active");
						jQuery(this).addClass("active");
						showGedFiles(jQuery(this).data("ged-path"));
					});

					jQuery(".ged-file-row").on("click", function () {
						jQuery("#ecm_file_rowid").val(jQuery(this).data("ged-id"));
						jQuery("#attach_selected_filename").text(jQuery(this).data("ged-name"));
						jQuery("#addedfile").val("");
						jQuery("#addfile").attr("name", "addfilefromged");
						jQuery("#attach_ged_modal").addClass("hidden");
					});

					jQuery("#addedfile").on("change", function () {
						var filename = this.files && this.files.length ? this.files[0].name : "Aucun fichier choisi";
						jQuery("#attach_selected_filename").text(filename);
						jQuery("#ecm_file_rowid").val("0");
						jQuery("#addfile").attr("name", "addfile");
					});

					jQuery("#addfile").on("click", function () {
						if (jQuery("#ecm_file_rowid").val() !== "0") {
							jQuery(this).attr("name", "addfilefromged");
						} else {
							jQuery(this).attr("name", "addfile");
						}
					});
				});
				</script>';
