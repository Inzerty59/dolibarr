...
$error = 0;
if (empty($reshook)) {
		// Add local PC file in ticket creation form
	if (($action == 'add' || $action == 'create') && GETPOST('addfile', 'alpha') && $permissiontoadd) {
	...
	if (($action == 'add' || $action == 'create') && GETPOST('addfilefromged', 'alpha') && $permissiontoadd) {
		require_once DOL_DOCUMENT_ROOT.'/core/lib/files.lib.php';
		...
		$ecmfileid = GETPOSTINT('ecm_file_rowid');

		if ($ecmfileid <= 0) {
			setEventMessages('Veuillez choisir un fichier GED.', null, 'errors');
			$action = 'create';
		} else {
			$sql = 'SELECT rowid, filepath, filename';
			$sql .= ' FROM '.MAIN_DB_PREFIX.'ecm_files';
			$sql .= ' WHERE rowid = '.((int) $ecmfileid);
			$sql .= ' AND entity IN ('.getEntity('ecm_files').')';

			$resql = $db->query($sql);
			if ($resql && ($objged = $db->fetch_object($resql))) {
				$sourcefull = DOL_DATA_ROOT.'/'.$objged->filepath.'/'.$objged->filename;
				...
				if (empty($realsource) || empty($realroot) || strpos($realsource, $realroot.DIRECTORY_SEPARATOR) !== 0 || !is_file($realsource)) {
					setEventMessages('Fichier GED introuvable ou non autorisé.', null, 'errors');
				} else {
					$vardir = $conf->user->dir_output.'/'.$user->id;
					$upload_dir_tmp = $vardir.'/temp';
					dol_mkdir($upload_dir_tmp);

					$destfile = dol_sanitizeFileName($objged->filename);
					$destfull = $upload_dir_tmp.'/'.$destfile;

					if (is_file($destfull)) {
						$info = pathinfo($destfile);
						$destfile = $info['filename'].'-'.dol_print_date(dol_now(), 'dayhourlog').(empty($info['extension']) ? '' : '.'.$info['extension']);
						$destfull = $upload_dir_tmp.'/'.$destfile;
					}

					$rescopy = dol_copy($sourcefull, $destfull, '0', 1, 0, 0);
					if ($rescopy > 0) {
				...
				setEventMessages('Fichier GED introuvable.', null, 'errors');
			}
	...
	// Remove attached file before ticket creation
	if (($action == 'add' || $action == 'create') && GETPOST('removedfile') && !GETPOST('removeAll') && $permissiontoadd) {
		require_once DOL_DOCUMENT_ROOT.'/core/lib/files.lib.php';

		dol_remove_file_process(GETPOSTINT('removedfile'), 0, 1, '');
		$action = 'create';
	}
