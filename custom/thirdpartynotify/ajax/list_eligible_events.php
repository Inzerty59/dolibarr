...
if (empty($token) || !hash_equals((string) currentToken(), (string) $token)) {
	thirdpartynotify_events_json(array('success' => false, 'error' => 'Bad token'), 403);
}

$socid = GETPOSTINT('socid');
if ($socid <= 0) {
	thirdpartynotify_events_json(array('success' => false, 'error' => 'Bad parameters'), 400);
}

if (!$user->hasRight('societe', 'lire')) {
	thirdpartynotify_events_json(array('success' => false, 'error' => 'Forbidden'), 403);
}
...
$sql .= " AND (c.code IS NULL OR c.code <> 'AC_OTH_AUTO')";
